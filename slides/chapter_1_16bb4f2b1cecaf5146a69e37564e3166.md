...
name: Prateek Narang
title: Co-founder, Coding Blocks
...
It can be useful for Text Classification & Clustering, Sentiment Analysis and Information Retrieval.
...
- So,BOW assumption that the feature vectors we are going to construct will be related to word frequencies and not to local positioning of words.{{2}}


`@script`
We also observe that these words appear at different positions in the sentence but convey the same positive sentiment about the product. So,BOW assumption that the feature vectors we are going to construct will be related to word frequencies and not to local positioning of words holds true.
...
`@script`
But there can be some potential problems with this assumption. You can see
the given review it contains features like "like", "good" which indicate positive sentiment but due to negation we know the overall sentiment is negative.
 
So to handle such cases, we can also introduce bigram features which means we will treat two consecutive words as a single feature.
...
`@script`
So let us now talk about Bag of Words Pipeline. We will start with data collection, followed by Tokenization, Stopwords Removal and Stemming, and then building a common vocabulary,finally vectorization before feeding into to our classifier. Let us understand these terms one by one.
...
You can see this example the effect of stopword removal.
